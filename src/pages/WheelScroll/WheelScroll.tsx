...
const useTransformStyle = ([scale, rotate]: StyleProp) => {
  const [[currentScale, currentRotate], setStyle] = useState<StyleProp>([scale, rotate]);
  // 一个延迟恢复设置缩放最大值
  const timerRef = useRef<NodeJS.Timeout | undefined>(undefined);
  const setTransformStyle = (setAction: StyleProp | ((prevStyle: StyleProp) => StyleProp)) => {
    setStyle((prevStyle) => {
      const [scale, rotate] = (typeof setAction !== 'function') ? setAction : setAction(prevStyle);
      // 缩放比率大于最大值， 延迟设置为最大值
